...
x <- rep(NA, ncol(stocks))
return_to_p <- stocks[-1,]
return_to_p <- rbind(return_to_p,x)
stocks$Agric_tp <- return_to_p$Agric
stocks <- stocks[-c(12196,12197,12198), ]
...
for (i in seq((initial_w_size+1),stocks_num,by=1)){ #fixed window size 
  ...
  actural_return <-  stocks$Agric_tp[i]
  predicted_return <- pred_value[[1]]
  f_err <- actural_return - predicted_return
...
Agric_linear_lag30<-ggplot(pred_lm_fix) + geom_line( aes( x =  Date, y = RMSE_diff, colour = "Mean -Linear")) 
ggsave("Agric_linear_lag30.jpg", plot = Agric_linear_lag30)

#save the outcome of computer industry-linear-lag365
write.csv(pred_lm_fix,'linear_Agriculture_lag30.csv')
...
for(j in window_size){#though he said don't snoop the windowsize outside the cV, I am now using cv to find the best window size
  ...
  for (i in seq((j+1), stocks_num, by=1)){ #fixed window size
    a <- (i-j)
    train_X <- model.matrix(Agric_tp ~., stocks[a:(i-1), ])[,-c(1,50)]
    predict_x <- model.matrix(Agric_tp ~., stocks[(i-1):i, ])[,-c(1,50)]
...
for (i in seq((initial_w_size+1), stocks_num, by=1)){ #fixed window size
  train_X <- model.matrix(Agric_tp ~., stocks[(i-initial_w_size):(i-1), ])[,-c(1,50)]
  predict_x <- model.matrix(Agric_tp ~., stocks[(i-1):i, ])[,-c(1,50)]
...
#save outcome
write.csv(pred_lasso,'pred_lasso_Agriculture_lag30.csv')
Agric_lasso_lag30<-ggplot(pred_lasso) + geom_line( aes( x =  Date, y = RMSE_diff, colour = "Mean - Lasso")) 
ggsave("Agric_lasso_lag30.jpg", plot = Agric_lasso_lag30)
...
for(i in window_size){
  ...
  for (leaf in min_leaf){
    for (j in seq((i + 1),stocks_num,by = 1)){
      ...
      rf <- randomForest(Agric_tp ~ ., train, ntree = 5,nodesize = leaf)
...
for (i in seq((initial_w_size+1), stocks_num, by=1)){ #fixed window size
  train <- stocks[(i-initial_w_size):(i-1), -c(1)]
  rf <- randomForest(Agric_tp ~ ., train, ntree = 5,nodesize = op_node_size)
...
#save outcome for rf
write.csv(pred_rf, "RF_Agriculture_lag30.csv")

Agric_rf_lag30<- ggplot(pred_rf) + geom_line( aes( x =  Date, y = RMSE_diff, colour = "Mean - Random Forest")) 
ggsave("Agric_rf_lag30.jpg", plot = Agric_rf_lag30)
...
for(j in window_size){#
  ...
  for (i in seq((j+1), stocks_num, by=1)){ #fixed window size 
    a <- (i-j)
    train_X <- model.matrix(Agric_tp ~., stocks[a:(i-1), ])[,-c(1,50)]
    predict_x <- model.matrix(Agric_tp ~., stocks[(i-1):i, ])[,-c(1)]
    train_Y <- c(stocks[a:(i-1),50])
    dat <- data.frame(train_X, train_Y)
    tune.out=tune(svm, Agric_tp~.,data=stocks[a:(i-1), -c(1)] ,kernel ="radial",
...
#save outcome
write.csv(pred_svm,'svm_Agriculture.csv')
Agric_svm_lag1<- ggplot(pred_svm) + geom_line( aes( x =  Date, y = RMSE_diff, colour = "Mean - Svm")) 
ggsave("Agric_svm_lag1.jpg",plot = Agric_svm_lag1)
